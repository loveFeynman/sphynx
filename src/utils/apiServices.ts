...
async function getTokenDetails(
  ...
  return { name, symbol, pair: `${symbol}/BNB` }
}

async function getTokenInfoForChart(input: any) {
  const minutes = 5
  const query = `{
...
async function getChartData(input: any, resolution: any) {
  const resolutionMap = {
    1: 1,
    5: 5,
    10: 10,
